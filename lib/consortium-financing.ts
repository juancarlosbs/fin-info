...
function calculateFinancing(input: FinancingInputs): FinancingResult {
  ...
  function calculatePricePayment(currentBalance: number, remainingMonths: number): number {
    if (remainingMonths <= 1) return currentBalance + currentBalance * monthlyRate
    return monthlyRate > 0
      ? (currentBalance * monthlyRate) / (1 - Math.pow(1 + monthlyRate, -remainingMonths))
  ...
  for (let month = 1; month <= term; month++) {
    ...
    const remainingMonths = term - month + 1
    const baseAmortization = input.system === "sac"
      ? (month === term ? balance : originalFinancedWithIof / term)
      : Math.max(0, calculatePricePayment(balance, remainingMonths) - interest)
    const scheduledAmortization = Math.min(balance, baseAmortization)
    const remainingAfterScheduledPayment = Math.max(0, balance - scheduledAmortization)
    const earlyAmortization = month === input.earlyAmortization.month
      ? Math.min(remainingAfterScheduledPayment, input.earlyAmortization.amount)
      : 0
    const amortization = scheduledAmortization + earlyAmortization
    ...
    // Juros pagos na parcela não aumentam o saldo devedor; somente correção/indexador
    // e amortização alteram o principal em aberto.
